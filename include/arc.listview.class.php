...
class ListView
{
    ...
    function GetPageListDM($list_len,$listitem="index,end,pre,next,pageno",$pagelang)
    {
        ...
        if($optionlen < 36) $optionlen = 36;
        if($optionlen > 100) $optionlen = 100;
        $optionlist = "<select name='sldd' onchange='location.href=this.options[this.selectedIndex].value;'>\r\n";
        for($mjj=1;$mjj<=$totalpage;$mjj++)
        {
            if($mjj==$this->PageNo)
            {
                $optionlist .= "<option value='".$purl."PageNo=$mjj' selected>$mjj/$totalpage</option>\r\n";
            }
            else
            {
                $optionlist .= "<option value='".$purl."PageNo=$mjj'>$mjj/$totalpage</option>\r\n";
            }
        }
        $optionlist .= "</select>";
