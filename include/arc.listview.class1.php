...
class ListView
{
     ...
     * @return    string
     */
    function __construct($typeid, $searchArr=array(), $uppage=1)//联动地区
    {
        global $dsql,$ftp;
        $this->TypeID = $typeid;
        $this->dsql = &$dsql;
		...
		$this->sAddTable = false;//联动地区
		$this->searchArr = $searchArr;//联动地区
  ...
  //获得附加表和列表字段信息//联动地区
			...
			if (isset($_REQUEST['tid']))
			{
				...
				foreach($_GET as $key => $value) {
					if ($key!="tid" && $key!="TotalResult" && $key!="PageNo") {
						$this->Fields[string_filter($key)] = ($k>0 ? ", " : ""). string_filter(urldecode($value));
					}
					$k++;
				}
			}
    ...
    //php4构造函数
    function ListView($typeid,$searchArr=array(),$uppage=0){
        $this->__construct($typeid,$searchArr,$uppage);
    }
...
if(count($this->searchArr) > 0)
{
	if(!empty($this->searchArr['nativeplace']))
	{
		...
		{
			$naddQuery .= " AND addf.nativeplace >= {$this->searchArr['nativeplace']} AND addf.nativeplace < ".($this->searchArr['nativeplace']+500)."";
		}
		else
		{
			if(preg_match("#\.#", $this->searchArr['nativeplace']))
			{
				...
				$naddQuery .= "AND addf.nativeplace = {$this->searchArr['nativeplace']}";
			}
			else
			{
				$naddQuery .= " AND addf.nativeplace >= {$this->searchArr['nativeplace']} AND addf.nativeplace < ".($this->searchArr['nativeplace']+1)."";
			}
		}
	}
}
...
if($this->sAddTable)
{
	$cquery = "SELECT COUNT(*) AS dd FROM `{$this->AddTable}` addf LEFT JOIN `#@__archives` arc ON arc.id = addf.aid  WHERE ".$this->addSql;
}
else
{
    ...
    $imgwidth=120,$imgheight=90,$listtype="all",$orderby="default",$innertext="",$tablewidth="100",$ismake=1,$orderWay='desc')
    {
        ...
        if($orderby=="senddate" || $orderby=="id") {
			...
			$ordersql = "  ORDER BY arc.weight $orderWay";//如果没有特定设置排序则按照权重先排序
		}
        else {
...
if($this->sAddTable)
{
	$addField = ',addf.'.join(',addf.',$this->ListFields);
        }
        else
        {
            $addField = '';
        ...
        if(preg_match('/hot|click|lastpost|weight/', $orderby)|| $this->sAddTable)
        {
        ...
        else
        {
            $t1 = ExecTime();
            $ids = array();
            $query = "SELECT id FROM `#@__arctiny` arc $addJoin WHERE {$this->addSql} $filtersql $ordersql LIMIT $limitstart,$row ";//本句有改动
    ...
    function GetPageListST($list_len,$listitem="index,end,pre,next,pageno",$pagelang)
    {
        ...
        $purl = $this->GetCurUrl();
        $maininfo = "<li><a class=\"\">".$uatotal." <strong>{$totalpage}</strong>".$uapage."<strong>".$this->TotalResult."</strong>".$uarecords."</a></li>";
        ...
        {
            $indexpage="<li class=\"thisclass\"><a>".$uahome."</a></li>";
        }
        ...
        {
            $endpage="<li class=\"thisclass\"><a>".$ualastpage."</a></li>";
        }
...
if($this->sAddTable)
{
	$geturl = "tid=".$this->TypeID."&TotalResult=".$this->TotalResult."&nativeplace=$nativeplace"."&";
}
else
{
     ...
     * @access    public
     * @return    string
	 * @return    {dede:itemcount listsize='4'/}
     */
