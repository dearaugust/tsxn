...
function lib_flink(&$ctag,&$refObj)
{
	...
	if(!$limit){
        ...
        $equery = "SELECT * FROM #@__flink $wsql order by sortrank asc limit $limit";
    }
    ...
    while($dbrow=$dsql->GetObject())
    {
		...
		$rbtext = preg_replace("/\[field:webname([\/\s]{0,})\]/isU", cn_substr($dbrow->webname,$titlelen), $rbtext);
		$rbtext = preg_replace("/\[field:logo([\/\s]{0,})\]/isU", $dbrow->logo, $rbtext);
		$rbtext = preg_replace("/\[field:link([\/\s]{0,})\]/isU", $link, $rbtext);
		$rbtext = preg_replace("/\[field:global.autoindex([\/\s]{0,})\]/isU", $GLOBALS['autoindex'], $rbtext);
		$GLOBALS['autoindex']++;
		$revalue .= $rbtext;
    }
    return $revalue;
