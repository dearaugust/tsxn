        ...
        $innertext='', $arcid=0, $idlist='', $channelid=0, $limit='', $att='', $order='desc', $subday=0, $noflag='',$tagid='', $pagesize=0, $isweight='N')
{
    ...
    if($idlist=='')
    {
        ...
        if($writer=='yes') {
            $wname =  isset($refObj->Fields['writer']) ? $refObj->Fields['writer'] : '';
			if($wname != '')
			{
				$orwheres[] = " arc.writer = '$wname' ";
			}
			else
			{
				$wmid =  isset($refObj->Fields['mid']) ? $refObj->Fields['mid'] : 0;
		...
		if($writer=='title') {
            $wtitle =  isset($refObj->Fields['title']) ? $refObj->Fields['title'] : '';
            $orwheres[] = " arc.writer = '$wtitle' ";
        }
