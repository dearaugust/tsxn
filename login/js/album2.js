...
function checkMuList2(psid,cmid){
...
//图集，显示与隐藏zip文件选项
function showZipField2(formitem,zipid,upid){
    if(formitem.checked){
...
//图集，显示与隐藏Html编辑框
function showHtmlField2(formitem,htmlid,upid){
    if($Nav()!="IE"){
		alert("该方法不适用于非IE浏览器！");
		return ;
	}
...
function seePicNewAlb2(f, imgdid, frname, hpos, acname){
    var newobj = null;
	if(f.value=='') return ;
	vImg = $Obj(imgdid);
...
//删除已经上传的图片(编辑时用)
function delAlbPicOld2(picfile, pid){
