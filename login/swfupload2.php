...
if(empty($dopost))
{
    ...
    //保存信息到 session
    if (!isset($_SESSION['file_info2'])) $_SESSION['file_info2'] = array();
    if (!isset($_SESSION['bigfile_info2'])) $_SESSION['bigfile_info2'] = array();
    if (!isset($_SESSION['fileid2'])) $_SESSION['fileid2'] = 1;
    else $_SESSION['fileid2']++;
    
    $_SESSION['bigfile_info2'][$_SESSION['fileid2']] = $fileurl;
    $_SESSION['file_info2'][$_SESSION['fileid2']] = $imagevariable;
    echo "FILEID:".$_SESSION['fileid2'];
    exit(0);
}
/************************
//生成缩图
...
else if($dopost=='thumbnail')
{
    ...
    if (!is_array($_SESSION['file_info2']) || !isset($_SESSION['file_info2'][$id]))
    {
        header('HTTP/1.1 404 Not found');
        exit(0);
    }
    header('Content-type: image/jpeg');
    header('Content-Length: '.strlen($_SESSION['file_info2'][$id]));
    echo $_SESSION['file_info2'][$id];
    exit(0);
}
/************************
//删除指定ID的图片
...
{
    if(!isset($_SESSION['bigfile_info2'][$id]))
    {
        echo '';
        exit();
    }
    $dsql->ExecuteNoneQuery("DELETE FROM `#@__uploads` WHERE url LIKE '{$_SESSION['bigfile_info2'][$id]}'; ");
    @unlink($cfg_basedir.$_SESSION['bigfile_info2'][$id]);
    $_SESSION['file_info2'][$id] = '';
    $_SESSION['bigfile_info2'][$id] = '';
    echo "<b>已删除！</b>";
    exit();
}
/************************
...
else if($dopost=='addtoedit')
{
	...
	echo $_SESSION['bigfile_info2'][$id];
	exit();
}
/************************
//获取本地图片的缩略预览图
